            ...
            checkDBifUserExists().then(
                x => {
                    if (docSnap != undefined && docSnap.exists()) {
                        ...
                        const docData2 = {
                            citationsArray: [], postsArray: []
                        }
                        // await setDoc(doc(db, "Users", address), docData2);
                        setDoc(doc(db, "Users", address), docData2).then(
                            x => {
                    ...
                    {!walletConnected ? (
                            ...
                            {users.map((user) => {
                                ...
                                return (
                                    <>
                                        <div>
                                            {" "}
                                            <b>{user.id.startsWith("0x") ? `user's address : ${user.id}` : ""}</b>

                                            <ul>
                                                {user.id.startsWith("0x") && user.citationsArray.map(item => {
                                                    return <li>{JSON.stringify(item)}</li>;
                                                })}
                                            </ul>
                                            <ul>
                                                {user.id.startsWith("0x") && user.postsArray.map(item => {
                                                    return <li>{JSON.stringify(item)}</li>;
                                                })}
                                            </ul>
                                        </div>
                        ...
                        <>
                            ...
                            {!userExists ? (
                                ...
                                <>
                                    let a= 1;
                                    let entries = Object.entries(currUser);

                                    <div>
                                        LOGIN SUCCESS() CALLED
                                        curr user data :
                                    ...
                                    <ul>
                                        {currUser.citationsArray && currUser.citationsArray.map(item => {
                                            return <li>{item[0]}</li>;
                                        })}
                                    </ul>
                                    <ul>
                                        {currUser.postsArray && currUser.postsArray.map(item => {
                                            return <li>{item[0]}</li>;
                                        })}
                                    </ul>
                                </>
